...
def get_isomer_suggestion(molecule):
 try:
    ...
    if(response.status_code==200):
        ...
        suggestions.pop()
        print(f"""\033[94m _________________________________________________\n\n The number of  possible Isomers is {len(suggestions)}\n_________________________________________________
        ""","\033[0m","\n".join(suggestions),'\n_________________________________________________')
        return suggestions
 except Exception:
...
def cross_check_molecule(llmgeneration, correct_molecule):
    ...
    llmcleaned = normalize_string(llmgeneration)
    print(options)
...
if(__name__=="__main__"):
    ...
    cross_check_molecule("acetic acid ethyl ester","Ethyl Acetate")